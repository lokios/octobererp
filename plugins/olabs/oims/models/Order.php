...
class Order extends Model
{
     ...
     */
    public $table = 'olabs_oims_orders';
    ...
    private function onOrderStatusChange($newOrderStatus, $prevOrderStatus) {
        ...
        if (isset($newOrderStatus)) {
            ...
            if ($newOrderStatus->mail_template != null) {
                Mail::send($newOrderStatus->mail_template->code, $data, function($message) use ($oimsSetting, $newOrderStatus) {

                    $message->to($this->contact_email);
                    
                    if ($oimsSetting->copy_all_order_emails_to != "") {
                        $message->bcc($oimsSetting->copy_all_order_emails_to);
                    }
