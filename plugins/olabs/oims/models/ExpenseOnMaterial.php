...
use BackendAuth;
/**
 * ExpenseOnMaterial Model
 */
class ExpenseOnMaterial extends BaseModel
{
     ...
     */
    public $table = 'olabs_oims_expense_on_materials';

    const CNAME = 'expenseonmaterials';
    ...
    public $hasMany = [
        'products' => [
            'Olabs\Oims\Models\ExpenseOnMaterialProduct', 
...
//        }
    ...
    /**
     * Complete create expenseOnMaterial from basket
     * - fill all
     * - create invoice
     * - 
     * 
    ...
    public function createFromBasket($basket) {
        ...
        // Payment method + ExpenseOnMaterialStatus
        //$this->payment_method = $basket["payment_method_id"]; // obsolete
        $paymentGateway = PaymentGateway::find($basket["payment_method_id"]);
        $this->paymentGateway = $paymentGateway;
        if (($paymentGateway) && ($paymentGateway->expenseOnMaterialStatusBefore)) {
            $this->objectstatus = $paymentGateway->expenseOnMaterialStatusBefore;
        }
        ...
        foreach ($basket["products"] as $id => $productJson) {
            ...
            if ($oimsSetting->extended_inventory_management) {
                // extended_inventory_management - after change expenseOnMaterial status
            }
            else {
                // check and remove qty form stock
                $qty = $product->expenseOnMaterialProduct($qty, true);
    ...
    public function genereateInvoice() { 
        $this->onExpenseOnMaterialStatusChange($this->objectstatus, null);
        ...
        $html =  str_replace("{{expenseOnMaterial_id}}", $this->id, $html);
       ...
       /* $html =  str_replace("{{total_price}}", $oimsSetting->getPriceFormatted($this->total_price + $this->shipping_price), $html); */
        ...
        $products = (count($this->products)) ? $this->products : ExpenseOnMaterialProduct::getPurchaseProducts($this->id);
    ...
    public function beforeUpdate() {
        ...
        // check expenseOnMaterial status change
        if (isset($oldModel)) {
            if (($oldModel->objectstatus == null) && ($this->objectstatus != null)) {
                ...
                if ($oldModel->objectstatus->id != $this->objectstatus->id) {
                    $this->onExpenseOnMaterialStatusChange($this->objectstatus, $oldModel->objectstatus);
                }
            }
        }
    }
    ...
    /**
     * After expenseOnMaterial status change - send email
     * 
     * @param type $newExpenseOnMaterialStatus
     * @param type $prevExpenseOnMaterialStatus
     */
    private function onExpenseOnMaterialStatusChange($newExpenseOnMaterialStatus, $prevExpenseOnMaterialStatus) {
        
        $oimsSetting = \Olabs\Oims\Models\Settings::instance();
        
        if (isset($newExpenseOnMaterialStatus)) {
            // prepair data
            $data = [];
            $data["first_name"] = $this->is_first_name;
            $data["last_name"] = $this->is_last_name;
            $data["expenseOnMaterial_id"] = $this->id;
            ...
            // send email
            if ($newExpenseOnMaterialStatus->mail_template != null) {
                Mail::send($newExpenseOnMaterialStatus->mail_template->code, $data, function($message) use ($oimsSetting, $newExpenseOnMaterialStatus) {

                    $message->to($this->contact_email);
                    
                    if ($oimsSetting->copy_all_expenseOnMaterial_emails_to != "") {
                        $message->bcc($oimsSetting->copy_all_expenseOnMaterial_emails_to);
                    }
                    
                    // attach invoice?
                    if (($newExpenseOnMaterialStatus->attach_invoice_pdf_to_email) && ($this->invoice != null)) {
            ...
            if ($oimsSetting->extended_inventory_management) {
                ...
                // qty decrease
                if ($newExpenseOnMaterialStatus->qty_decrease) {
                    foreach ($this->products_json as $key => $producJson) {
                        $product = \Olabs\Oims\Models\Product::find($producJson["product_id"]);
                        if ($product) {
                            $qty = $producJson["quantity"];
                            $product->expenseOnMaterialProduct($qty, true);
                        }
                    }
                }
                
                // qty increase back
                if ($newExpenseOnMaterialStatus->qty_increase_back) {
                    // !!! ONLY if have previous status with qty_decrease
                    if (($prevExpenseOnMaterialStatus != null) && ($prevExpenseOnMaterialStatus->qty_decrease)) {
                        foreach ($this->products_json as $key => $producJson) {
                            $product = \Olabs\Oims\Models\Product::find($producJson["product_id"]);
                            if ($product) {
                                $qty = $producJson["quantity"];
    ...
    /**
     * Find expenseOnMaterial for payment gateway or return null
     * 
     * @param type $id
     */
    public static function findExpenseOnMaterialBySlugForPaymentGateway($id) {
        $expenseOnMaterial = self::find($id);
        
        // check if expenseOnMaterial have user => user have to be logged in
        if ($expenseOnMaterial->user_id) {
            $user = \RainLab\User\Facades\Auth::getUser();
            ...
            // check another user
            if ($expenseOnMaterial->user_id != $user->id) {
                return null;
            }
        }
        
        // check disallowed expenseOnMaterial statuses
        if ($expenseOnMaterial->objectstatus) {
            if ($expenseOnMaterial->objectstatus->disallow_for_gateway) {
                return null;
            }
        }
        
        return $expenseOnMaterial;
