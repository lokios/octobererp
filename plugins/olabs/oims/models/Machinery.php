 ...
 * Machinery Model
 */
class Machinery extends BaseModel
{
     ...
     */
    public $table = 'olabs_oims_machineries';

    const CNAME = 'machineries';
    ...
    public $hasMany = [
        'products' => [
            'Olabs\Oims\Models\MachineryProduct', 
            'key' => 'machinery_id'
        ],  
    ];
...
//        }
    ...
    /**
     * Complete create machinery from basket
     * - fill all
     * - create invoice
     * - 
     * 
    ...
    public function createFromBasket($basket) {
        ...
        // Payment method + MachineryStatus
        //$this->payment_method = $basket["payment_method_id"]; // obsolete
        $paymentGateway = PaymentGateway::find($basket["payment_method_id"]);
        $this->paymentGateway = $paymentGateway;
        if (($paymentGateway) && ($paymentGateway->machineryStatusBefore)) {
            $this->objectstatus = $paymentGateway->machineryStatusBefore;
        }
        ...
        foreach ($basket["products"] as $id => $productJson) {
            ...
            if ($oimsSetting->extended_inventory_management) {
                // extended_inventory_management - after change machinery status
            }
            else {
                // check and remove qty form stock
                $qty = $product->machineryProduct($qty, true);
    ...
    public function genereateInvoice() { 
        $this->onMachineryStatusChange($this->objectstatus, null);
        ...
        $html =  str_replace("{{machinery_id}}", $this->id, $html);
       ...
       /* $html =  str_replace("{{total_price}}", $oimsSetting->getPriceFormatted($this->total_price + $this->shipping_price), $html); */
        ...
        $products = (count($this->products)) ? $this->products : MachineryProduct::getPurchaseProducts($this->id);
        ...
        if (isset($oldModel)) {
            if (($oldModel->objectstatus == null) && ($this->objectstatus != null)) {
                ...
                if ($oldModel->objectstatus->id != $this->objectstatus->id) {
                    $this->onMachineryStatusChange($this->objectstatus, $oldModel->objectstatus);
                }
            }
        }
    }
    ...
    /**
     * After machinery status change - send email
     * 
     * @param type $newMachineryStatus
     * @param type $prevMachineryStatus
     */
    private function onMachineryStatusChange($newMachineryStatus, $prevMachineryStatus) {
        
        $oimsSetting = \Olabs\Oims\Models\Settings::instance();
        
        if (isset($newMachineryStatus)) {
            // prepair data
            $data = [];
            $data["first_name"] = $this->is_first_name;
            $data["last_name"] = $this->is_last_name;
            $data["machinery_id"] = $this->id;
            ...
            // send email
            if ($newMachineryStatus->mail_template != null) {
                Mail::send($newMachineryStatus->mail_template->code, $data, function($message) use ($oimsSetting, $newMachineryStatus) {

                    $message->to($this->contact_email);
                    
                    if ($oimsSetting->copy_all_machinery_emails_to != "") {
                        $message->bcc($oimsSetting->copy_all_machinery_emails_to);
                    }
                    
                    // attach invoice?
                    if (($newMachineryStatus->attach_invoice_pdf_to_email) && ($this->invoice != null)) {
            ...
            if ($oimsSetting->extended_inventory_management) {
                ...
                // qty decrease
                if ($newMachineryStatus->qty_decrease) {
                    foreach ($this->products_json as $key => $producJson) {
                        $product = \Olabs\Oims\Models\Product::find($producJson["product_id"]);
                        if ($product) {
                            $qty = $producJson["quantity"];
                            $product->machineryProduct($qty, true);
                        }
                    }
                }
                
                // qty increase back
                if ($newMachineryStatus->qty_increase_back) {
                    // !!! ONLY if have previous status with qty_decrease
                    if (($prevMachineryStatus != null) && ($prevMachineryStatus->qty_decrease)) {
                        foreach ($this->products_json as $key => $producJson) {
                            $product = \Olabs\Oims\Models\Product::find($producJson["product_id"]);
                            if ($product) {
                                $qty = $producJson["quantity"];
    ...
    /**
     * Find machinery for payment gateway or return null
     * 
     * @param type $id
     */
    public static function findMachineryBySlugForPaymentGateway($id) {
        $machinery = self::find($id);
        
        // check if machinery have user => user have to be logged in
        if ($machinery->user_id) {
            $user = \RainLab\User\Facades\Auth::getUser();
            ...
            // check another user
            if ($machinery->user_id != $user->id) {
                return null;
            }
        }
        
        // check disallowed machinery statuses
        if ($machinery->objectstatus) {
            if ($machinery->objectstatus->disallow_for_gateway) {
                return null;
            }
        }
