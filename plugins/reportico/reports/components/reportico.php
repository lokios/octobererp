...
class reportico extends reportico_object
{
    ...
    */
	...
	function get_execute_mode()
	{
		...
		if ( !$this->execute_mode && \Session::has(reportico_namespace().".execute_mode")  )
		{
                ...
                if ( isset($this->initial_execution_parameters[$col->query_name]) )
                {
                    ...
                    if ( $col->criteria_type == "DATERANGE" )
                    {
                        ...
                        else
                        {
                            ...
                            {
                                \Session::put(reportico_namespace().".latestRequest.MANUAL_".$col->query_name."_FROMDATE", $val1);
                                \Session::put(reportico_namespace().".latestRequest.MANUAL_".$col->query_name."_TODATE", $val2);
                            }
                        }
                    }
                    else if ( $col->criteria_type == "DATE" )
                    {
                        ...
                        else
                        {
                            ...
                            {
                                \Session::put(reportico_namespace().".latestRequest.MANUAL_".$col->query_name."_FROMDATE", $val1);
                                \Session::put(reportico_namespace().".latestRequest.MANUAL_".$col->query_name."_TODATE", $val1);
                                \Session::put(reportico_namespace().".latestRequest.MANUAL_".$col->query_name, $val1);
                            }
                        }
                    }
                    else
...
//echo "check1";
				...
				{
//echo "check1 SW_ADMIN_PASSWORD".SW_ADMIN_PASSWORD;
//echo "req SW_ADMIN_PASSWORD".$_REQUEST['admin_password']."<BR>";
...
//var_dump(\Session::all());
        ...
        if ( preg_match("/_drilldown(.*)/", reportico_namespace(), $matches) )
        {
            ...
            if ( \Session::has($parent_session.'.project_password') )
            {
                set_reportico_session_param('project_password', \Session::get($parent_session.".project_password"));
            }
        }
...
//echo "ass $this->reportico_ajax_script_url\n";
	...
	function handle_xml_query_input($mode=false)
	{
        ...
        if ( $do_defaults)
        {
		    ...
		    global $g_project;
            $old_error_handler = set_error_handler("\Reportico\Reports\Components\ErrorHandler", 0);
            ...
            $old_error_handler = set_error_handler("\Reportico\Reports\Components\ErrorHandler");
        }
...
//var_dump(\Session::all());
        ...
        if ( !\Session::has($g_session_namespace_key) || isset($_REQUEST['clear_session']) || $this->clear_reportico_session)
		...
		$old_error_handler = set_error_handler("\Reportico\Reports\Components\ErrorHandler");
        set_exception_handler("\Reportico\Reports\Components\ExceptionHandler");
...
$code = "namespace Reportico\Reports\Components;". $code;
...
function save_admin_password($password1, $password2, $language)
{
    ...
    if ( strtoupper($password1) == "NOPASSWORD" )
    {
        $password1 = "__OPENACCESS__";
        $password2 = "__OPENACCESS__";
    }

//echo "SAVE !!!! $this->admin_projects_folder <BR>";
		...
		if ( AJAX_ENABLED )
		{
			$tag = "";
       			...
       			if ( preg_match ( "/TODATE/", $in_tag ) )
              			$tag .= "";
			$tag .= '<input  class="'.$this->lookup_query->getBootstrapStyle('textfield').'swDateField" id="swDateField_'.$in_tag.'" style="z-index: 500" type="text" name="MANUAL_'.$in_tag.'"';
			$tag .= ' size="20"';
			$tag .= ' maxlength="20"';
			$tag .= ' value="'.$in_value.'">';
			$text .= $tag;
