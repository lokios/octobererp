...
{section name=menu loop=$DROPDOWN_MENU_ITEMS}
                    <li class="dropdown reportico-dropdown-listitem"><a class="dropdown-toggle reportico-dropdown-link nav-label" data-toggle="dropdown" href="#">{$DROPDOWN_MENU_ITEMS[menu].title}<span class="caret"></span></a>
...
{if strlen($ADMIN_MENU_URL)>0} 
              <li style="display:inline-block">
                    <a class="nav-link swAdminButton2 reportico-dropdown-link" href="{$ADMIN_MENU_URL}">{$T_ADMIN_HOME}</a>
              </li>
{/if}
{/if}
